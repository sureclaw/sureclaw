...
export class IdentityModule extends BasePromptModule {
  ...
  private renderEvolutionGuidance(_ctx: PromptContext): string[] {
    return [
      ...
      '',
      'Identity files live in your workspace under `.ax/identity/`. To update one,',
      'read the current content first, then use `write_file` to save the new version.',
      'Changes are committed automatically after each turn — do not run git commands.',
      '',
      '### When to Evolve',
      '',
