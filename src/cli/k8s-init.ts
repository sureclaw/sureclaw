...
const VALID_WASM_MODES = ['enabled', 'shadow', 'disabled'];
...
export function parseArgs(args: string[]): InitOptions {
  ...
  for (let i = 0; i < args.length; i++) {
    ...
    switch (arg) {
      ...
      case '--wasm': opts.wasm = next(); break;
...
/** Return the default WASM mode for a given preset when the user doesn't specify one. */
export function defaultWasmMode(preset: string): string {
  switch (preset) {
    case 'small': return 'enabled';   // dev/test — go fast
    case 'medium': return 'shadow';   // production rollout — observe first
    case 'large': return 'enabled';   // production performance
...
export function generateValuesYaml(opts: {
  ...
  // WASM fast path (inside config block)
  const wasmMode = opts.wasm ?? defaultWasmMode(opts.preset);
  if (wasmMode !== 'disabled') {
    lines.push('  wasm:');
    lines.push(`    enabled: ${wasmMode === 'enabled'}`);
    lines.push(`    shadow_mode: ${wasmMode === 'shadow'}`);
  }
