...
**Tags:** agent-ux, prompt-engineering, toolgen, documentation, load-bearing, codegen

### Defensive input guards must not reject the no-args case when all params are optional
**Date:** 2026-04-19
**Context:** Codegen's runtime guard threw `TypeError: listTeams expects a single object argument` on `listTeams()` calls — but all of `list_teams`'s params (limit, cursor, orderBy, ...) are optional, and no-args is a legitimate "list everything" call. The guard was written as `if (params === null || typeof params !== 'object') throw`, which catches the string-arg mistake it was designed to catch AND the undefined-from-no-args case. Agents interpret the thrown TypeError the same way they interpret the IPC error it replaced — "I got the shape wrong" — and retry with arbitrary new shapes.
**Lesson:** When adding a defensive argument guard to a generated function, split the decision by whether any parameter is required. If `schema.required?.length === 0`, emit `fn(params = {})` so the undefined case is normalized to an empty object before the guard runs. If any param IS required, keep the bare `fn(params)` so missing-required still surfaces the actionable "expects a single object argument" error. The `required` array is the exact signal that distinguishes the two cases. **How to apply:** any time a generated-code guard rejects `typeof x !== 'object'`, check whether the zero-arg invocation should be valid (it is for pure list/enumerate operations, it isn't for operations that need an ID). Default the param when it is, leave it bare when it isn't. Add a test per branch so the guard's breadth can't silently regress.
**Tags:** codegen, toolgen, agent-ux, runtime-guard, defensive-programming, optional-params
