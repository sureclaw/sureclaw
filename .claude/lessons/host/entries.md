...
### Module-level loggers in session-long agent processes go stale across turns
**Date:** 2026-04-23
**Context:** PR #187 (chat observability) review caught that `src/agent/runner.ts`, `runners/claude-code.ts`, and `runners/pi-session.ts` all bound their reqId at module import time via `const logger = getLogger().child({ reqId: process.env.AX_REQUEST_ID?.slice(-8) })`. Pods in k8s HTTP work-loop mode (and stdin-mode session sandboxes) process MULTIPLE turns per pod lifetime — each turn ships a fresh `requestId` in its payload, but the captured-at-load module logger still stamped attempt-1's reqId on every line. `grep <reqId>` worked for the first turn and went dark for every turn after.
**Lesson:** When a long-lived process re-enters a work loop with new per-turn correlation IDs, module-level loggers MUST be rebound, not captured. Pattern: `let logger!: ...` (definite-assignment assertion) + a `rebindReqLogger()` helper that reads the current env value, called once at module load AND from the work-loop dispatcher (here: `applyPayload` in runner.ts updates `process.env.AX_REQUEST_ID` then calls rebind; each runner submodule calls rebind at its own entry point). The signal that this bug exists in your code: a `const logger = ...child({ reqId: ... })` at module scope inside any process that handles multiple correlated requests over its lifetime. **How to apply:** before binding a correlation ID into a module-level logger, ask "does this process handle exactly one such ID?" If no, rebind from a known dispatch point — don't trust `process.env` to be a stable identity.
**Tags:** logger, correlation, reqId, runner, agent, session-long-pods, module-state, observability

### Per-attempt failure trackers must clear state at the top of each attempt, not just at terminal
**Date:** 2026-04-23
**Context:** PR #187 review caught that `WaitFailureTracker.lastRecord` was never cleared between attempts. Scenario: attempt 0 records `agent_response_timeout`, attempts 1+2 fail without recording into the tracker (e.g. agent process exits non-zero without rejecting the response promise). The terminal `emitTerminal` would surface attempt 0's `agent_response_timeout` as the cause — but timeout did NOT kill the chat; the silent non-zero exit on attempt 2 did. Operator sees a misleading reason.
**Lesson:** When a tracker collapses N per-attempt records into one terminal event by "most recent wins", the freshness window MUST be one attempt — not the whole loop. Add a `startAttempt()` method (or `clear()` if the loop won't enforce naming) and call it at the top of every retry iteration. The pattern: tracker.startAttempt() → attempt body → if attempt-failed: tracker.record(...) → loop. Then tracker.emitTerminal at exhaustion. Without `startAttempt`, a recorded cause from the FIRST failed attempt persists into the terminal emit even if the actual final attempt failed for a different reason. The fix is one method + one call site, but the bug is invisible until the specific "fails-then-fails-differently-without-recording" pattern hits prod. **How to apply:** review every multi-attempt tracker / collector that exposes "most recent" semantics — if there's no per-attempt clear, the recorded value can leak across attempts and misattribute outcomes.
**Tags:** retry-loop, tracker, per-attempt, observability, exactly-once, chat-termination

### Per-stream level filters must floor across opts.level AND env overrides
**Date:** 2026-04-23
**Context:** PR #187 review caught that the multistream/transport per-stream `level` filter used `consoleStreamLevel = opts.level ?? getMinConfiguredLevel()`. Production (cli/index.ts) DOES pass `opts.level` explicitly. So a user setting `LOG_LEVEL_SANDBOX_K8S=debug` while the CLI ran with `opts.level: 'info'` would see the component child's debug emits dropped at the per-stream filter — exactly the symptom the multistream fix from Task 7's review was supposed to prevent. The bug only manifests when the caller passes opts.level AND env has a more-permissive override.
**Lesson:** When floor-ing a stream's level for a multi-source level resolution scheme, the floor must be the MOST PERMISSIVE (lowest numeric) of every input — not just one. `opts.level ?? envFloor` is wrong; `LEVEL_NUMERIC[opts.level] < LEVEL_NUMERIC[envFloor] ? opts.level : envFloor` is right. The intuition: explicit caller intent sets the ROOT pino level (which gates the no-component default-callers), but the per-stream floor must allow ANY child that opted into a more-permissive component override to still emit. The child's own `level` is the actual gate; the per-stream filter just admits.
**Tags:** logger, multistream, log-levels, env-overrides, level-resolution
