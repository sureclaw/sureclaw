...
- **IPC tools** (`ipc-tools.ts` for pi-session, `mcp-server.ts` for claude-code): Non-sandbox tools proxy to host via IPC -- `memory_*`, `web_*`, `audit_query`, `identity_write`, `user_write`, `scheduler_*`, `skill_propose`, `request_credential`, `agent_delegate`, `image_generate`, `workspace_*`, `identity_propose`, `proposal_list`, `agent_registry_list`. Each calls `client.call({action, ...params})`. All IPC consumers use the `IIPCClient` interface (not concrete `IPCClient`), making them transport-agnostic.
- **`request_credential` is a standalone tool**: Lives in `tool-catalog.ts` with `category: 'credential'`. Always available (not filtered by skill presence). Maps to `credential_request` IPC action. Used when skills or web APIs need env vars the agent doesn't have.
