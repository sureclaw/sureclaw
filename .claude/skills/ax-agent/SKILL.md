...
| `src/agent/workspace-cli.ts` | CLI for container provision/cleanup phases (GCS restore, scope diff, release) | CLI entry point (`provision`, `cleanup` commands) |
...
- **IPC tools** (`ipc-tools.ts` for pi-session, `mcp-server.ts` for claude-code): Non-sandbox tools proxy to host via IPC -- `memory_*`, `web_*`, `audit_query`, `identity_write`, `user_write`, `scheduler_*`, `skill_*`, `skill_import`, `skill_search`, `agent_delegate`, `image_generate`, `workspace_*`, `identity_propose`, `proposal_list`, `agent_registry_list`. Each calls `client.call({action, ...params})`. All IPC consumers use the `IIPCClient` interface (not concrete `IPCClient`), making them transport-agnostic.
- **Sandbox tools routing** (two paths):
  - **Container mode** (docker, apple, k8s): `local-sandbox.ts` executes bash/file ops inside the agent's own container. Protocol: `sandbox_approve` IPC call to host for audit gate -> execute locally (execFileSync/readFileSync/writeFileSync) -> `sandbox_result` IPC call (best-effort). Uses `safePath()` for path containment. Enabled when `CONTAINER_SANDBOXES.has(config.sandboxType)`.
...
- **Web proxy bridge**: Both runners detect `AX_WEB_PROXY_SOCKET` (container mode) or `AX_WEB_PROXY_URL`/`AX_WEB_PROXY_PORT` (k8s/subprocess) env vars and start a loopback TCP bridge (`web-proxy-bridge.ts`) that forwards to the host proxy. Sets `HTTP_PROXY`/`HTTPS_PROXY`/`http_proxy`/`https_proxy` env vars for child processes (npm, curl, git, etc.).
