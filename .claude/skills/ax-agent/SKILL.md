...
- **IPC tools** (`ipc-tools.ts` for pi-session, `mcp-server.ts` for claude-code): Non-sandbox tools proxy to host via IPC -- `memory_*`, `web_*`, `audit_query`, `identity_write`, `user_write`, `scheduler_*`, `skill_propose`, `skills_index`, `request_credential`, `agent_delegate`, `image_generate`, `workspace_*`, `identity_propose`, `proposal_list`, `agent_registry_list`. Each calls `client.call({action, ...params})`. All IPC consumers use the `IIPCClient` interface (not concrete `IPCClient`), making them transport-agnostic.
