...
| ST-8 | Structural | PASS | Comprehensive unit tests covering lifecycle, CRUD, persistence, cron, runOnce, dedup, scheduleOnce, heartbeat, async cleanup, rehydration |
...
**Result:** PASS
**Evidence:**
- `src/host/provider-map.ts:77` — `plainjob: '../providers/scheduler/plainjob.js'`
...
**Result:** PASS
**Evidence:**
- No matches for `proactive|hint|token_budget|confidence_threshold` as logic in plainjob.ts
...
**Result:** PASS
**Evidence:**
- `tests/providers/scheduler/plainjob.test.ts` — 32+ tests (SQLiteJobStore unit + scheduler provider)
...
- scheduleOnce: "scheduleOnce fires job via setTimeout and auto-deletes", "job can be cancelled via removeCron"
...
- Rehydration: "one-shot jobs are rehydrated on start", "past-due jobs fire immediately", "rehydration skips jobs belonging to other agents"
...
- Sent `hello` via `ax send` — agent responded: "Hello. How can I help you today?"
- No scheduler errors in `ax.log`

### BT-2: Scheduler database created on startup
**Result:** PASS
**Evidence:**
- `$TEST_HOME/data/scheduler.db` exists after server startup (4096 bytes)
...
**Result:** PASS
**Evidence:**
- Inserted one-shot job with future run_at: `'oneshot-persist-1', '* * * * *', 'main', 'One-shot reminder', 1, '2026-03-05T19:56:21.000Z'`
- Stopped server, restarted
- Queried: `SELECT id, run_once, run_at ... WHERE id = 'oneshot-persist-1'` — returned `oneshot-persist-1|1|2026-03-05T19:56:21.000Z`
