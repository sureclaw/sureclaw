...
function CredentialRequestCard({ req, onChange }: CredentialRequestCardProps) {
  ...
  const handleSave = useCallback(async () => {
    ...
    setError(null);
    try {
      await api.provideCredential(req.envName, value, req.sessionId);
      setSubmitting(false);
      setSuccess(true);
      successTimerRef.current = setTimeout(() => {
        onChange();
  ...
  }, [value, req.envName, req.sessionId, onChange]);

  return (
    <div
      className="card"
      data-testid={`credential-request-${req.sessionId}-${req.envName}`}
    >
      <div className="card-header flex items-start justify-between gap-4">
        <div className="min-w-0">
          <div className="flex items-center gap-2">
            ...
            <span className="inline-flex items-center gap-1 rounded-full border border-sky/20 bg-sky/5 px-2 py-0.5 text-[10px] font-medium text-sky">
              Credential Requested
            </span>
          </div>
          <p className="mt-1 text-[12px] text-muted-foreground">
            <span className="font-medium text-foreground/80">{req.agentName}</span>{' '}
            is asking for this credential.
            <span className="ml-2 font-mono text-[11px] text-muted-foreground/60">
              session {req.sessionId.slice(0, 10)}…
            </span>
          </p>
        </div>
      </div>
      ...
      <div className="card-body space-y-3">
        ...
        {error && (
          <div className="flex items-start gap-2 p-2.5 rounded-lg bg-rose/5 border border-rose/15">
            <AlertTriangle size={14} className="text-rose shrink-0 mt-0.5" />
            <p className="text-[13px] text-rose break-words">{error}</p>
          </div>
        )}
        ...
        <div className="flex items-center justify-end gap-3 pt-1">
          ...
          <button
            onClick={handleSave}
            disabled={submitting || success || value.trim() === ''}
            className="btn-primary text-[13px] flex items-center gap-1.5"
          >
            {submitting ? (
              <>
...
export default function SkillsPage() {
  ...
  } = useApi<CredentialRequestsResponse>(() => api.credentialRequests(), []);
  ...
  const requests = credRequests?.requests ?? [];
  const loading = (setupLoading && !setup) || (credLoading && !credRequests);
  const nothingPending = agentGroups.length === 0 && requests.length === 0;
      ...
      <div className="flex items-end justify-between animate-fade-in-up">
        <div>
          ...
          <p className="mt-1 text-[13px] text-muted-foreground max-w-2xl">
            Skills need a few things before they can talk to the outside world: some
            domains on the allowlist, and some credentials we can inject into
            requests. Approve them here and we'll wire the rest.
          </p>
        </div>
        <button
      ...
      {!loading && nothingPending && (
        <div className="flex flex-col items-center justify-center py-16 text-center">
          ...
          <h3 className="text-[14px] font-semibold text-foreground mb-1">
            All your skills are set up. Nice.
          </h3>
          <p className="text-[13px] text-muted-foreground max-w-md">
            Every installed skill is set up and happy. Nothing to approve.
          </p>
        </div>
      )}
    </div>
